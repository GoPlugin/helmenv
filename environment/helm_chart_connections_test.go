...
func TestURLsByPort(t *testing.T) {
	...
	remoteURLs, err := e.Charts.Connections("plugin").RemoteURLsByPort("access", environment.HTTP)
	require.NoError(t, err)
	localURLs, err := e.Charts.Connections("plugin").LocalURLsByPort("access", environment.HTTP)
	require.NoError(t, err)
	// Try collecting URLs 5 times and make sure they're consistently ordered
	for i := 0; i < 5; i++ {
		newRemotes, err := e.Charts.Connections("plugin").RemoteURLsByPort("access", environment.HTTP)
		require.NoError(t, err)
		newLocals, err := e.Charts.Connections("plugin").LocalURLsByPort("access", environment.HTTP)
		require.NoError(t, err)
